...
| `POST /api/hitl/{hitl_id}` | `{decision: "approved"\|"rejected"}` | updated `hitl_requests` row in **SQLite**; publishes SSE `hitl_request`. **Honesty:** decide is persisted today; it does **not** relay/pause a live AgentOS run (relay = future work). Until then, treat HITL as operator audit + SQLite status, not AgentOS control. | `ARCNET_WRITE_SECRET` when set |
