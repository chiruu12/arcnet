...
- [x] Live test: hand a Case File to a coding agent → pull traces via **HTTP** (`GET /api/signoz/evidence`, Query Range, Case File `links.signoz_trace`) — **SHIPPED:** `scripts/verify_mcp_handoff.py` (gate G5 HTTP path). SigNoz MCP stdio remains optional PARTIAL (`deploy/mcp/diag_stdio.sh`); not required for the demo beat.
