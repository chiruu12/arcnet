...
Hero screen = **Time Machine**: two-column baseline-vs-candidate replay diff + a terminal-style verdict readout + recommendation with `hand_to(claude_code)` / `replay_corpus()` actions.
