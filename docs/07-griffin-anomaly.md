...
- **Phase-7 P7-B LIVE-VERIFIED (2026-07-24):** one real cycle 123.7s on CPU (load + fit + predict, cached weights), estimator flipped to `tabfm` honestly, no degrade — [`_phase7_p7b_live.json`](_phase7_p7b_live.json). Default runtime remains MAD (opt-in env).
- **Phase-7 P7-B ship (2026-07-24):** opt-in `ARCNET_TABFM=1` starts daemon async worker (N=1 @ 360s); `tabfm_worker.forecast(backend=tabfm)` lazy-loads weights; any failure → MAD degrade for process lifetime; `GET /api/griffin/status` reports `estimator=tabfm` only after a successful forecast + additive `tabfm{enabled, loaded, last_forecast_ms, degrade_reason}`. Default runtime unchanged (MAD). Driver live-verify with cached weights required for full exit.
