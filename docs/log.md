...
## 2026-07-24 — P11 wave: adversarial hardening landed (API red-team + concurrency + write-auth) + gpt-5.6-luna

**Three composer worktree runs, driver-gated + hand-composed; opencode still out on balance.**
P11-A — `server/arcnet_server/validation.py` (`parse_json_object` → 400 on empty/malformed/non-object, `require_fields`, `bound_*` body caps: transcript 1 MiB, text 16 KiB, hitl/findings 64 KiB → 422 on oversize); every write POST routed through it; agent-view `goal` echo excerpted in `agent_session_context`/`session_check_data`; adversarial suite `test_redteam_api.py` (malformed / path-injection / pagination-abuse / oversize / header-abuse / write-auth-bypass); `docs/30-hardening.md`.
P11-B — thread-safety pass: `bus.py` asyncio.Queue → thread-safe bounded `queue.Queue` (drop-oldest, lock-guarded fan-out; fixes latent cross-thread `put_nowait` from worker threads), `db.py` `_ThreadSafeConnection` proxy + `RLock` held across cursor drain + `busy_timeout=5000`, `griffin.py` cache `RLock` + deepcopy snapshot + `shutdown_background_workers()`, `main.py` double-checked `get_conn` init + SSE reads via `run_in_executor`; `test_concurrency.py` (10 races, no deadlock, 0.56s).
P11-C — `server/arcnet_server/write_auth.py` (`require_write_auth`: writes open by default, else `X-Arcnet-Write-Secret`/Bearer via `hmac.compare_digest` → 401 `{detail,hint}`; separate `ARCNET_WEBHOOK_SECRET`); applied to every mutating POST incl. apply-model/replay/hitl/griffin-evaluate; `docs/32-deployment-notes.md` ("how far from production" map); README limitations synced.
**Hand-composition (A+C overlap on POST handlers, A+B both add docs/30):** each handler ends up `require_write_auth(request)` + `parse_json_object`/`bound_*`/`require_fields`; docs/30 merged (both sections). **Cross-packet fixes:** A's `test_apply_model_not_gated_by_write_secret` flipped to `..._gated` (C correctly gates apply-model as a mutating route); `scripts/e2e_product_coherence.py` `_next_hitl` migrated off the removed asyncio.Queue await to the new blocking `queue.Queue.get` (bus API change surfaced only in scripts/, outside the packet suites).
**gpt-5.6-luna (user request):** added to `model_catalog.py` (now 19 models, integrity clean, frontier_reasoning, GPT-5-class list-price estimate) and SDK `model_explore.py` snapshot + tool_heavy/injection_resist prefer lists; live via `/model-intel`. Runtime `ARCNET_MODEL` stays `legacy-baseline-v1` (real callable model — luna is a catalog/recommendation entry, not wired to run).
**Final gate on main:** server **207** / agents 18 / sdk 14 / hq 40 · boundaries clean · coherence PASS · services restarted; live smoke PASS (malformed→400, missing-fields→400, valid write→200, localhost-trust logged, luna in model-intel candidates). All pushed (`1cb944f` merge, `f167218` model).
**Honesty:** held at **~64 (cap ≤65)** — P11 is robustness/hardening (measured: +42 adversarial server tests, live smoke), not a new user-facing capability or a closed live-check DEFER, so no % move.

## 2026-07-24 — P12: deep replay hardening (Time Machine path)

**One composer worktree run, driver-gated.** Target chosen after reading `replay_service.py` end to end: the headline demo path had five ways to 500 on hostile/corrupt stored data.
`replay_service.py` — `_json()` now catches `json.JSONDecodeError` → `{}` (malformed stored transcript/outcome/usage no longer crashes; `execute_replay` still raises `ValueError` when the transcript is genuinely absent); new `_as_int`/`_as_float` coercion helpers replace bare `int()`/`float()` on stored fields (a non-numeric `"many"` in usage was a 500); `transcript.steps` and `divergences` list-guarded with `isinstance(step, dict)` filters in the comprehensions; `execute_replay` validates each AgentOS run payload is a dict → `ValueError("candidate runtime returned malformed run N")`. All five now degrade to clean 422s via the route's existing `except ValueError` mapping; mid-replay HTTP failure still propagates to 502 with **no partial verdict persisted** (asserted via `mock_verdict.assert_not_called()`).
`server/tests/test_replay_hardening.py` — 12 offline tests (AsyncMock'd `httpx.AsyncClient`, never touches live AgentOS): malformed JSON, numeric fallbacks, malformed-run ValueError, mid-replay failure, degenerate `{}`×3 runs still producing a valid verdict shape with `confidence == "3/3 runs"`.
**Driver catch:** the full server suite flagged `test_r2_r3_surface.py::test_list_and_recommend` — my own luna prefer-list change had shifted the top recommendation, and I'd only re-run the model-intel + sdk suites when landing it. Fixed forward (`12107a8`, assert luna first *and* gpt-4o still present). Standing rule now: **any SDK change re-runs the full server suite** — server tests import SDK modules.
**Final gate on main:** server **219** / agents 18 / sdk 14 / hq 40 · boundaries clean · coherence PASS · uvicorn restarted on new code; live smoke PASS (health OK, replay on unknown session → 404). Pushed (`8aeea6b`).
**Honesty:** held at **~64 (cap ≤65)** — hardening, not new capability.

## 2026-07-25 — recording-day rehearsal + P13 HQ resilience

**Demo path re-verified live after P11/P12** (which rewrote the bus, every write handler, and the replay path). Blocker found and cleared: Docker was down, and once up `signoz-signoz-0` crash-looped on `failed to validate config "user"` — `foundryctl cast` regenerates the compose and drops `SIGNOZ_USER_ROOT_PASSWORD` (the gotcha already documented in `docs/plans/capture-checklist.md`). Re-injected into the gitignored generated compose; UI healthy in 35s.
**Silent trace loss (new finding, documented):** while the SigNoz backend crash-loops, OTLP `:4318` still accepts posts and returns 200, but spans never reach ClickHouse — newest traces were 3 days stale and a scenario run in that window recorded to SQLite with **no trace to open**. After bringing SigNoz healthy, a re-run wrote +15 spans (`arcnet-scenarios` 132 → 147). Checklist now says: confirm `(healthy)` + `ui_reachable: true` *before* any run you intend to show.
**Live re-verification, all PASS:** SSE (`/signals/stream`) delivers on write with no refresh; `POST /api/replay` on Worms → 30s, `r_40210783`, `mixed`, `3/3 runs`; live S1 → exfil blocked and reproduces hero `s_ecfdb55d` exactly (**corrected below: 3 threats, not 10**); trace waterfall `Agent_J.run → OpenAIChat.invoke → send_email/lookup_customer/fetch_url` plus `arcnet.guard` spans carrying rule/pattern_class/action; SigNoz `ui_reachable` + `query_range_ok` + 4/4 dashboards.
**Narration correction recorded:** live Worms replay is `mixed`, candidate ~12.8× the cost ($0.0109 vs $0.00085) for one fewer step, and step count moves run to run — Edgar is the honest Shot 5 hero (baseline `failed` → candidate `clean`).
**P13 (composer, driver-gated):** HQ had no resilience layer at all. 11 real crash sites fixed — `App.tsx` `f.map()` on non-array fleet, session-pagination `push(...data)`, `FleetHealth` `a.health.sessions_24h` on null health, `page.rows.map` across every paged view, `.toUpperCase()` on malformed signal/hitl/verdict rows, `TimeMachine` `verdict.divergences.map`, `CaseFiles` `recommended_actions.map`, `HqAgent` `tl.versions`, `res.json()` on HTML/502 bodies, unguarded SSE `JSON.parse`. New `apiResilience.ts` (error-envelope parsing, `asArray`/`asNum`, row normalizers, SSE guard) + `ViewErrorBoundary` per view. Happy-path visuals unchanged. **hq 40 → 60 tests.** Deferred: per-view retry, agent-view schema validation, SSE offline indicator, case-file download errors, browser E2E.
**Final gate:** server **219** / agents 18 / sdk 14 / hq **60** · boundaries clean · coherence exit 0 · `pnpm build` clean · all HQ modules transform clean on the live vite server.
**Honesty:** held at **~64 (cap ≤65)** — resilience and rehearsal, not new capability.

## 2026-07-25 — P14: measured guard coverage (honest number)

**Composer packet, measurement only — no guard code, thresholds, or production behaviour changed.**
Built a deterministic offline corpus (`sdk/tests/fixtures/guard_corpus.json`, 42 entries across 10 public taxonomy classes), ran every payload through `build_guard()` exactly as production configures it, and recorded checkpoint / rule / category / risk per payload. Report: `docs/33-guard-coverage.md`. Regression-locked in `sdk/tests/test_guard_corpus.py` so coverage cannot silently regress.
**Headline: caught 25 of 38 synthetic attack payloads (65.8%).** 3 classes fully covered on this corpus (`indirect_retrieved`, `destructive_tool`, `output_leakage`), 7 partial, 4 benign controls passed with no false blocks.
**Driver-added framing (the number alone is misleading in BOTH directions).** Each payload is scanned at ONE checkpoint in isolation, so a "miss" is not an end-to-end failure — ArcNet's defense is layered. Measured with a fresh `build_guard()` per call: `direct_override_new_instructions` is `allow`/0.0 at both USER and RETRIEVED input, but the same payload in a tainted `send_email` side effect is **blocked** (`retrieved_source_in_side_effect`, 0.85). **Taint tracking, not the regex/heuristic input layer, is the load-bearing defense** — the trust-boundary architecture claim, now measured. 9 of 13 misses are input-checkpoint-only.
**The one real residual hole:** untainted content leaving via a tool side effect is `allow`/0.0 at `tool_call` (`exfil_untainted_send_email_gap`). Output scanning blocks the same content (`email_address` 0.8; raw SSN → `ssn` 0.8) and the tainted variant blocks at 0.85 — but a side effect does not necessarily pass through output scanning, so user-origin secrets or a taint-provenance miss get out. Listed first under Known gaps for a future packet.
**Methodology correction recorded in the doc:** my first ad-hoc probe reused one `Guard` instance and reported `review`/0.35 for the untainted tool-call case; that was contamination (the crescendo rule accumulates trajectory state across scans). Fresh-guard numbers are the ones published.
**Gate:** sdk **14 → 63** (+49 corpus tests) · server 219 · agents 18 · hq 60 · boundaries clean.
**Honesty:** held at **~64 (cap ≤65)** — P14 measures what already existed; it changes our knowledge, not the product. The newly documented gap was always present, just unquantified.

## 2026-07-25 — unplug 0.6.0 + a correction to the S1 threat profile

**Correction to the entry above (driver error, caught by the P17 packet).** The claim "same 10 threats incl. `injection / ignore_previous`" was **wrong**. `GET /api/threats` accepts `since` / `agent_id` / `limit` / `offset` — there is **no `session_id` parameter**, and FastAPI silently ignores unknown query params. Passing `?session_id=…&limit=10` returned the first 10 rows of the **fleet-wide** list (110 threats across 33 sessions), which is trivially identical between any two sessions. The reproduction conclusion still holds, but the real numbers are:

| Session | Threats |
|---|---|
| hero `s_ecfdb55d` | **3** |
| live rerun `s_0d446be7` | **3**, identical rules + scores |

Actual S1 profile: `taint`/`retrieved_source_in_side_effect` 0.85 (`tool_call`, block) · `trajectory`/`crescendo_block` 0.92 (`tool_call`, block) · `trajectory`/`crescendo_block` 0.92 (`output`, redact). **`injection / ignore_previous` and `leakage / email_address` are not in the S1 chain at all** — the poisoned page scans `allow` / 0.0 at `retrieved`, so nothing fires at ingest and the attack is stopped at the side effect. Session-scoped reads must use `GET /api/agent-view/threats/{session_id}` or `repository.threats_for_session`. `video-script.md` Shot 2 and `capture-checklist.md` corrected — the script had been telling the operator to narrate a rule name that never fires on camera.

**P17 — `unplug-ai` 0.5.2 → 0.6.0 (composer eval, driver-verified independently).** Behavioral **no-op** on every measured dimension: corpus **25/38 (65.8%) identical**, zero improvements, **zero regressions**, zero false positives on benign controls, and every `rule` / `category` / `pattern_class` / `risk_score` byte-identical on all 25 caught payloads. API surface unchanged (`Guard`, `GuardConfig`, `Source`, `TrustLevel`, `TaintedText`, `Action`, `ScanResult`; same method signatures; `guard_verdict_from_result` mapping intact). Hero S1 chain identical on both versions. Driver did not take "zero delta across a minor bump" on trust — independently confirmed `unplug.__version__ == 0.6.0` in the worktree venv and re-ran the corpus by hand before merging. Pins bumped in `sdk/pyproject.toml` + `agents/pyproject.toml`; `docs/33` guard-version line updated; measured numbers unchanged because the run was identical.
**Honesty:** held at **~64 (cap ≤65)**.

**0.6.0 verified live on the demo path.** Both uvicorns restarted so the agent process actually loads 0.6.0 (the guard runs inside AgentOS — a merge alone does not change what a running service imports). Live S1 on 0.6.0 → `s_c0127a30`, `blocked_email: true`, and **the same 3 threats** as the hero: `retrieved_source_in_side_effect` 0.85, `crescendo_block` 0.92 (`tool_call`), `crescendo_block` 0.92 (`output`). Full gate on 0.6.0: server **219** / agents 18 / sdk 63 · boundaries clean · coherence OK.

**Test-environment finding (not a code regression, worth fixing post-submission):** `server/tests/test_tabfm_worker_stub.py::test_tabfm_backend_unavailable_without_weights` **hangs indefinitely — it does not fail — when `tabfm` is installed in the venv.** The test calls `forecast(series, backend="tabfm")` and asserts the unavailable/MAD-degrade path, but with the package present that call attempts a real weight load instead of short-circuiting (`import tabfm` alone costs **16.2s**). This cost ~20 minutes of misattributed debugging: the symptom (server suite hanging right after an unplug bump) looked exactly like a dependency regression, and it was actually the `uv pip install tabfm torch` that follows every `uv sync`. **The canonical test env must NOT have tabfm installed** — reinstall it only for a live TabFM verification run, then remove it. Proper fix: the test should force the unavailable path (monkeypatch the import or gate on an env flag) rather than depending on the package being absent, and it needs a hard timeout so it fails loudly instead of hanging.

## 2026-07-25 — TabFM coexistence (P18) + completeness audit (P19)

**P18 — the suite is now correct with OR without `tabfm` installed.** The hang documented above was fixed properly rather than by uninstalling the package: `test_tabfm_backend_unavailable_without_weights` now forces the degrade path via `patch_tabfm_unavailable()` (mocks `tabfm` in `sys.modules` and makes `_load_tabfm_model` raise) instead of relying on the package being absent, plus a `timeout_seconds(5.0)` SIGALRM guard so this class of failure can never hang again. New `server/tests/tabfm_test_support.py`; `setUp`/`tearDown` reset model state between tests. One production addition — `reset_tabfm_model_state_for_tests()` in `tabfm_worker.py` — is additive only and never called on a runtime path. **Measured both ways: 219 passed with tabfm absent AND 219 passed with tabfm installed (2.91s on main).** The opt-in no-import guarantee still holds: `test_no_tabfm_package_import_when_disabled` passes with the package present, and live `/api/griffin/status` with `ARCNET_TABFM` unset reports `estimator: mad`, `tabfm.enabled: false`.
**TabFM re-verified live on unplug 0.6.0 (driver):** real forecast, weights loaded **from local cache with no network**, `status: ready`, `estimator: tabfm`, prediction `10.937`, **143.6s** (vs 123.7s in P7-B — slower under concurrent load, same order). TabFM and unplug 0.6.0 coexist correctly.

**P19 — completeness audit → `docs/34-completeness-audit.md`.** Every claimed feature verified against code with `file:line` evidence. Buckets: **38 SHIPPED · 14 PARTIAL · 4 MISSING · 9 EXPLICIT DEFER.** Verdict: the P0 demo loop is shippable; the product is **not** complete against the full v2 spec. Independently, the audit found **~64% readiness is supported, not inflated** — the honesty regime holds up under scrutiny.
**Driver spot-checked the two most consequential claims before merging:** `canary` has **zero references** anywhere in `sdk/ server/ agents/ hq/src` (F9 genuinely MISSING, the unplug canary API was never wired), and `agents/` contains **no HITL handling** (the pause UI exists but nothing relays a decision to AgentOS). Both confirmed.
**Docs found overstating (recorded, not silently fixed):** `docs/15` claims no URL router and unrendered `guidance` — both stale (`hq/src/hash.ts`; `Signals.tsx:207`); `docs/02` shows HITL → AgentOS when the code is SQLite-only; `docs/20` test counts stale; `docs/22` marks P7-B "DONE" without noting TabFM is opt-in; demo script promises fleet latency the cards do not show.
**Top gaps (full ranked list of 14 in `docs/34` §11):** 1) hackathon capture (human) · 2) live hero replay re-verify · 3) G5 MCP live handoff · 4) HITL does not stop AgentOS · 5) dashboard UUID deep-links · 6) corpus scorecard · 7) prompt-swap replay UI · 10) F9 canaries.
**Gate:** server **219** (with tabfm installed) / agents 18 / sdk 63 / hq 60 · boundaries clean. **Honesty:** held at **~64 (cap ≤65)** — audits and test fixes, not new capability.

## 2026-07-25 — P20 doc truth pass (docs/README only)

Closed the last two gaps from the P19 audit (§11 #13 doc hygiene, #14 README verification). Docs and README only — no product code, no test logic, and **no readiness percentage moved** (verified on main: README + `docs/20` still pin **~64% / ≤65%**).
Fixed with re-verified evidence: `docs/15` claimed no URL router and unrendered `guidance` — both stale (`hq/src/hash.ts:12-27`, `Signals.tsx:185-207`), plus a sweep of that file's status column against P8–P14 reality; `docs/02` described HITL flowing to AgentOS — now states plainly that HQ approve/reject **updates SQLite only, with no relay from `decide_hitl` to AgentOS** (`repository.py:734-742`, `main.py:1055-1057`, zero HITL matches in `agents/`) and the `pause` signal is labelled a **scaffold** that does not pause or resume a live run; `docs/20` counts refreshed to 219/18/63/60; `docs/22` P7-B now carries the `ARCNET_TABFM=1` opt-in / MAD-default qualifier; `docs/06` no longer promises fleet-card latency the UI doesn't render. `docs/33` + `docs/34` are now linked from the README index, Limitations, Verification, and `docs/20`.
...
**2026-07-25 — Model catalog v2 + recommendation buckets (pre-record).** Rebuilt model catalog (`2026-07b`), bucketed recommendations, `GET /api/models/catalog`, HQ bucket UI, version timeline, ArcNet MCP server. **Honesty:** held at **~64 (cap ≤65)**.

**2026-07-26 — P36 OTLP ingest (any-framework door).** `POST /v1/traces` standard OTLP/HTTP (protobuf + JSON, ≤4 MiB): OpenInference spans grouped by `session.id` (else per-trace) under `arcnet.agent_id`/`service.name`, merged incrementally into agents/sessions (tokens, tool calls/errors, model, goal excerpt, trace_id). Observe-only — guard/replay still need the SDK. New `server/arcnet_server/otlp_ingest.py` (+`opentelemetry-proto` server dep), docs/36 quickstart, docs/12 additive row, README section. Live-verified with a real `opentelemetry-sdk` http/protobuf exporter: AGENT/LLM/TOOL trace landed in fleet with exact tokens (342/187/529) and `error` status from a failing tool span. Coherence script repaired: catalog pin `"2026-07"` → `model_catalog.CATALOG_VERSION` (broken by the 2026-07e bump) and HITL BUS drain past the P21 ack signal (broken since P21, unrun). **Gate: server 260 / sdk 88 / agents 25 / scripts 13 / hq 106 · boundaries + coherence PASS.** Honesty held ~64 (cap ≤65).
