...
export function parseHash(raw: string = window.location.hash): HashState {
  ...
  return { view, agent, session, model };
}

/** Build `#view?agent=&session=&model=` — omits empty query keys. */
...
/** Navigate to a view, optionally carrying agent/session/model filters. */
